...
export function formatPickleStepTitle(
  testStep: TestStep,
  ...
  return builder
    .append(
      new TextBuilder(stream)
        ...
        .append(formatPickleStepText(testStep, pickleStep, theme, stream))
        .build(theme.status?.all?.[status])
    )
    .build()
}

function formatPickleStepText(
  testStep: TestStep,
  pickleStep: PickleStep,
  theme: Theme,
  stream: NodeJS.WritableStream
...
export function formatSourceLocation(
  withSourceReference: Hook | StepDefinition | undefined,
  theme: Theme,
  stream: NodeJS.WritableStream
) {
  if (withSourceReference?.sourceReference.uri) {
    const builder = new TextBuilder(stream)
      .append('#')
      .space()
      .append(withSourceReference.sourceReference.uri)
    if (withSourceReference.sourceReference.location) {
      builder.append(':').append(withSourceReference.sourceReference.location.line)
    }
    return builder.build(theme.location)
  }
}
